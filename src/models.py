...
class BrainNetwork(nn.Module):
    def __init__(self, 
                 h=512,  # reduced from 4096
                 in_dim=15724, 
                 out_dim=512,  # reduced from 768
                 seq_len=2, 
                 n_blocks=2,  # reduced from 4
                 drop=.25,    # increased dropout
                 clip_size=768, # using smaller CLIP
                 blurry_recon=True,  # disable memory-intensive reconstruction
                 clip_scale=1):
        super().__init__()
