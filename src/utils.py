...
# Modified utils.mixco_nce function
def mixco_nce(preds, targs, temp=0.1, perm=None, betas=None, select=None, distributed=False, 
              accelerator=None, local_rank=None, bidirectional=True):
    # Force consistent precision and device, and ensure contiguous memory layout
    device = preds.device
    dtype = preds.dtype
    
    # Make tensors contiguous to avoid potential stride issues
    preds = preds.contiguous()
    targs = targs.to(dtype).to(device).contiguous()
    
    # Ensure temp is a tensor of the same dtype/device
    temp = torch.tensor(temp, dtype=dtype, device=device)
    
    # Compute similarity with explicit casting
    brain_clip = torch.matmul(preds, targs.T) / temp
